...
const (
	createEventQuery = `INSERT INTO tb_events (name_event, description_event, location_event, date_event) VALUES ($1, $2, $3, $4) RETURNING id_event`
)
...
func (s createEventAdapter) Execute(event *entities.Event) (*int, error) {
	...
	var eventId int
	err = tx.QueryRow(createEventQuery, event.Name, event.Description, event.Location, event.Date).Scan(&eventId)
	if err != nil {
		return nil, err
	}

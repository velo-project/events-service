...
type SuspendEventHandler struct {
	db   *sql.DB
	grpc *grpc.ClientConn
}

func NewSuspendEventHandler(db *sql.DB, grpc *grpc.ClientConn) *SuspendEventHandler {
	return &SuspendEventHandler{db: db, grpc: grpc}
}

func (h *SuspendEventHandler) Handle(c *gin.Context) {
	...
	suspendEventAdapter := database.NewSuspendEventAdapter(h.db)
	userExistsByIdPort := grpcadapter.NewUserExistsByIdAdapter(h.grpc)
	service := services.NewSuspendEventService(suspendEventAdapter, userExistsByIdPort)

	input := services.SuspendEventServiceInput{
		EventId: eventId,
		UserId:  userId,
	}
	...
	if err != nil {
		...
		if errors.Is(err, customErrors.ErrBlockedSuspendEvent) {
